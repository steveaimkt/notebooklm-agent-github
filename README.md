...
```
notebooklm-agent/
├── clients/                    # MCP 클라이언트 스크립트
│   ├── mcp_client.mjs          # 전체 기능 CLI (npx 기반)
│   ├── mcp_client_local.mjs    # 로컬 빌드 테스트용
│   └── mcp_client_debug.mjs    # 디버그용 (브라우저 표시)
├── examples/                   # 사용 예제
│   ├── ask_question.mjs        # 단일 질문 예제
│   └── multi_notebook_research.mjs  # 멀티 노트북 리서치 예제
├── .vscode/
│   └── mcp.json                # VS Code/Antigravity MCP 설정
├── .gitignore
└── README.md
```
