...
**11단계 마법사**가 한 번에 한 단계씩 안내합니다.

```
/start
```

진행 흐름 (시연 슬라이드와 1:1 매칭):
```
A. MCP 설치 (3분)          1. 환경 점검 → 2. npx 캐시 → 3. Google 인증 → 4. 헬스체크
B. 노트북 준비 (4분, 수동) 5. 노트북 생성 + 소스 3종 추가 (PDF + URL + 텍스트)
C. 에이전트 질문 (5분)     6. 등록 → 7. 기본 지정 → 8. 첫 질문(소스 귀속) → 9. 멀티패스 1→2→3
D. 자동 리포트 (3분)       10. auto-report.mjs → output/market_report.md (6섹션 통합)
E. 마무리                   11. 산출물 정리 + 재활용 안내
```
