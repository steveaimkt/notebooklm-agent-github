---
name: 상세페이지토론
description: 사용자가 "상세페이지토론하자", "상세페이지 토론 시작", "마케터 디자이너 토론해줘", "와이어프레임 만들어줘" 등으로 요청할 때 트리거. Pain Point + 공감 키워드를 입력받아 마케터·디자이너 두 에이전트가 각각 6섹션 기획안을 산출하고 심판이 채점·하이브리드 통합 → Figma 플러그인 호환 JSON까지 자동 생성한다.
---

# /상세페이지토론 — 마케터 vs 디자이너 토론 → Figma 와이어프레임 JSON 마법사

"상세페이지토론하자"가 입력되면 사용자가 처음 진입한 것으로 보고 **STEP 0**부터 시작합니다.
...
### 경로 A — 후속 모드 (권장)
`/상세페이지기획` 스킬의 산출물이 `output/`에 있을 때. STEP 0-A에서 자동 감지.
- `output/pain_point_analysis.md` — Pain Point 분석 (Stage 1 대체)
- `output/empathy_keyword_map.md` — 공감 키워드 맵 (Stage 2 대체)
- `output/creative_brief.md` — 핵심 메시지 / 차별화 (참고용)
...
**모드 A 선택 시:** 다음 파일들의 존재를 확인.
- `{work_root}/output/pain_point_analysis.md` — 필수
- `{work_root}/output/empathy_keyword_map.md` — 필수
- `{work_root}/output/creative_brief.md` — 권장

→ 필수 파일 미존재 시: "선행 산출물이 없습니다. `/상세페이지기획`을 먼저 실행하거나 모드 B로 진행하세요." 출력 후 STEP 0으로 복귀.
...
final_plan은 20~24개 섹션이 권장. 마케터 섹션 6개 + 디자이너 섹션 6개를 그대로 합치지 말고,
각 섹션의 강점만 골라 hybrid로 통합한 풍부한 와이어프레임으로 확장.
```

**검증 ★:** 채점 합계 / 평결 / final_plan 섹션 수를 사용자에게 미리보기. 섹션이 너무 적으면(15개 미만) 재실행 권장.
...
- `data.layout.children` 배열 길이 ≥ 15
- 각 섹션의 `children`에 TEXT/IMAGE_AREA/BUTTON 요소가 1개 이상
- 첫 섹션이 `Section_01_*`로 시작하는지
